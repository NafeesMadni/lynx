...
LayoutNode::LayoutNode(int id, const starlight::LayoutConfigs& layout_configs,
  ...
  measure_context_.font_scale_sp_only_ = layout_configs.font_scale_sp_only_;
  css_align_with_legacy_w3c_ = layout_configs.css_align_with_legacy_w3c_;
...
void LayoutNode::ConsumeStyle(CSSPropertyID id, const tasm::CSSValue& value,
                              bool reset) const {
  if (LayoutComputedStyleSetter::SetValue(id, value, reset, measure_context_,
                                          parser_configs_, *css_style_,
