...
using FunctionCreator = base::expected<jvalue, std::string> (*)(int);
...
class MethodInvoker : public std::enable_shared_from_this<MethodInvoker> {
 public:
  ...
  base::expected<std::unique_ptr<pub::Value>, std::string> Invoke(
 ...
 private:
  ...
  base::expected<std::unique_ptr<pub::Value>, std::string> Fire(
