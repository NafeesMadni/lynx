...
LynxModuleAndroid::InvokeMethod(const std::string& method_name,
  ...
  base::expected<std::unique_ptr<pub::Value>, std::string> value =
...
void LynxModuleAndroid::EnterInvokeScope(
  ...
  scope_timing_collectors_.push_back(
...
void LynxModuleAndroid::ExitInvokeScope() {
  ...
  if (!scope_timing_collectors_.empty()) {
    scope_timing_collectors_.pop_back();
  }
}
...
LynxModuleAndroid::CreateLynxNativePromise(
  ...
  auto executor_function_impl =
    ...
    if (!ret.has_value()) {
      auto error_message = BUILD_JSI_NATIVE_EXCEPTION(ret.error());
