...
bool MethodInvoker::ReportPendingJniException() {
  ...
  //            module_name_, method_name_, "Unable to get pending JNI
  //            exception."),
  //        "This error is caught by native, please ask RD of Lynx or client for
  //        " "help.", base::LynxErrorLevel::Error});
  //    return true;
  //  }
  ...
  //                        "This error is caught by native, please ask RD of
  //                        Lynx " "or client for help.",
...
base::expected<std::unique_ptr<pub::Value>, std::string> MethodInvoker::Invoke(
  ...
  auto ret = Fire(env, module, java_arguments);
  if (!ret.has_value()) {
    return base::unexpected("Module Failed");
  }
  LOGI("LynxModule, MethodInvoker::InvokeMethod, method: ("
       << module_name_ << "." << method_name_ << "." << first_arg_str
       << ") did fire " << this)
...
base::expected<std::unique_ptr<pub::Value>, std::string> MethodInvoker::Fire(
...
#define PRIMITIVE_CASE(METHOD)                                                 \
  {                                                                            \
    ...
    if (ReportPendingJniException()) {                                         \
      return base::unexpected(jni_error_hit);                                  \
    }                                                                          \
...
#define OBJECT_CASE(CLASS)                                                     \
  ...
  switch (return_type) {
    ...
    default:
      ...
      return base::unexpected("Unknown return type: " +
